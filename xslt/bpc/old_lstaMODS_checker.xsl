...
<axsl:stylesheet xmlns:axsl="http://www.w3.org/1999/XSL/Transform"
    ...
    version="1.0" mets:dummy-for-xmlns="" mods:dummy-for-xmlns="" rdf:dummy-for-xmlns="" xlink:dummy-for-xmlns="" exclude-result-prefixes="rdf sch xlink mets mods xsl cdl dc dcterms">
    <axsl:output method="xml"/>
    <xsl:template match="/">
        <xsl:call-template name="errors"/>
    </xsl:template>
    <axsl:template name="errors">
        <MetsBestPracticeErrors xsl:exclude-result-prefixes="mets xsl axsl sch mods rdf xlink">
    ...
    <axsl:template match="mets:dmdSec[1]" priority="4000" mode="M6">
        ...
        <axsl:choose>
            <axsl:when test="/mets:mets/mets:dmdSec[@ID='mods']/mets:mdWrap[@OTHERMDTYPE='MODS']"/>
            <axsl:otherwise>
