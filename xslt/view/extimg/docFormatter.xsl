...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  xmlns:xlink="http://www.w3.org/TR/xlink" xmlns:mets="http://www.loc.gov/METS/"
  xmlns:m="http://www.loc.gov/METS/" xmlns:cdl="http://ark.cdlib.org/schemas/appqualifieddc/"
  xmlns:n2="http://www.loc.gov/EAD/" xmlns:dcterms="http://purl.org/dc/terms/"
  xmlns:dc="http://purl.org/dc/elements/1.1/" version="2.0" exclude-result-prefixes="xlink mets xsl cdl dc dcterms">
  <xsl:output method="html"/>
  <xsl:output doctype-public="-//W3C//DTD XHTML 1.0 Transitional//EN" doctype-system="http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"/>
  <xsl:variable name="page" select="/"/>
  <xsl:variable name="layout" select="document('imageDisplayStructure.xml')"/>
  <xsl:template match="/">
    <xsl:apply-templates select="$layout/html"/>XX
  </xsl:template>
  <xsl:template match="@*|*">
    <xsl:copy>
  ...
  <xsl:template match="insert-head-title">
    <title>OAC: <xsl:value-of select="$page/mets:mets/@LABEL"/>
    </title>
  </xsl:template>
  <xsl:template match="insert-breadcrumb">
    <a
    href="http://findaid.oac.cdlib.org/search.image.html">Images</a>&#160;&#160;&gt;&#160; <a>
  ...
  <xsl:template match="m:div" mode="structMapImage">
    <xsl:variable name="link"> http://ark.cdlib.org/<xsl:value-of select="$page/m:mets/@OBJID"/>
      <xsl:text>/</xsl:text>
      <xsl:choose>
    ...
    <xsl:if test="m:div/m:fptr[@FILEID='thumbnail']">
      <a href="{$link}">
        <img src="/{$page/m:mets/@OBJID}/thumbnail" border="0" alt="Thumbnail image"/>
      </a>
    </xsl:if>
    <p>
      <span class="listingTitle">View Options:</span>
      <br/>
      <xsl:apply-templates select="m:div[m:fptr/@FILEID !='thumbnail']" mode="structMapLink"/>
    </p>
  </xsl:template>
  <xsl:template match="m:div" mode="structMapLink">
    <a href="/{$page/m:mets/@OBJID}/{./m:fptr/@FILEID}">
      <xsl:apply-templates select="@LABEL" mode="cleanup"/>
    </a>
    <br/>
  </xsl:template>
...
-->
    ...
    <h4 class="h{../@from}">
      <xsl:value-of select="../n2:did/n2:unittitle"/>
    </h4>
  </xsl:template>
  ...
  <xsl:template match="n2:series" mode="link1">
    ...
    <a href="{$link}">
      <xsl:value-of select="n2:unittitle"/>
    </a>
  </xsl:template>
