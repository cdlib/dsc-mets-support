...
<xsl:include href="./insert-md-portion-mods.xslt"/>
...
<xsl:variable name="divDepth" select="'3'">
 <!-- xsl:for-each select="$page/m:mets/m:structMap//m:div">
  <xsl:sort select="-1 * count(ancestor::m:div)" data-type="number" />
   <xsl:if test="position()=1">
    <xsl:value-of select="count(ancestor-or-self::m:div)"/>
   </xsl:if>
  </xsl:for-each -->
</xsl:variable>
...
<xsl:template match="insert-prevORDER">
 <xsl:if test="$thisDiv/preceding::m:div[m:div/m:fptr]/@LABEL[position()=last()]"> 
  <xsl:variable name="link">
	<xsl:text>/</xsl:text>
	<xsl:value-of select="$page/m:mets/@OBJID"/>
...
<img src="http://oac.cdlib.org/images/previous.gif" width="72" height="22" border="0" alt="&lt;&lt; previous" class="prevNextButton" align="middle"/><br/>
	<xsl:value-of select="($thisDiv/preceding::m:div[m:div/m:fptr]/@LABEL)[position()=last()]"/>
 </a>
 </xsl:if>
</xsl:template>

<xsl:template match="insert-nextORDER">
 <xsl:if test="$thisDiv/following::m:div[m:div/m:fptr]/@LABEL">
  <xsl:variable name="link">
	<xsl:text>/</xsl:text>
	<xsl:value-of select="$page/m:mets/@OBJID"/>
	<xsl:text>/?order=</xsl:text>
 ...
 <a href="{$link}">
<img src="http://www.oac.cdlib.org/images/next.gif" width="51" height="22" border="0" alt="next &gt;&gt;" class="prevNextButton" align="middle"/><br/>
 <xsl:value-of select="$thisDiv/following::m:div[m:div/m:fptr]/@LABEL"/>
 </a>
 </xsl:if>
</xsl:template>
...
<xsl:variable name="class">
  <xsl:choose>
	<xsl:when test="$page/m:mets/@TYPE='image collection'">breadcrumb-img</xsl:when>
...
<xsl:variable name="url">
  <xsl:choose>
	<xsl:when test="$page/m:mets/@TYPE='image collection'">http://www.oac.cdlib.org/search.image.html</xsl:when>
...
<xsl:variable name="oactype">
  <xsl:choose>
	<xsl:when test="$page/m:mets/@TYPE='image collection'">Images</xsl:when>
...
&#160;&#160;&gt;&#160; 
 ...
 <a href="http://www.oac.cdlib.org/findaid/ark:/{substring-after($page/m:mets/m:dmdSec[@ID='DMR2']/m:mdRef/@*[local-name(.)='href'],'ark:/')}">
...
<xsl:variable name="endsheet">
  <xsl:choose>
    <xsl:when test="(count($page/m:mets/m:structMap/m:div/m:div))
			&lt;
		    (25*number($sheet))">
	<xsl:value-of select="count($page/m:mets/m:structMap/m:div/m:div)"/>
    </xsl:when>
    <xsl:otherwise>
		...
		<xsl:value-of select="$page/m:mets/m:structMap/m:div/m:div[position()=$startsheet]/@LABEL"/> to  
                <xsl:value-of select="$page/m:mets/m:structMap/m:div/m:div[position()=$endsheet]/@LABEL"/> 
		 </xsl:otherwise>
		</xsl:choose>
	</xsl:when>
	<xsl:when test="$debug">
...
<xsl:template match="m:structMap" mode="sheet">
	<xsl:apply-templates select="m:div" mode="sheet"/>
</xsl:template>
...
<xsl:variable name="startsheet">
	<xsl:value-of select="25*(number($sheet) - 1)"/>
</xsl:variable>
<xsl:variable name="endsheet">
	<xsl:value-of select="25*number($sheet)"/>
</xsl:variable>
...
<xsl:choose>
  <xsl:when test="not(m:div[m:div/m:div/m:fptr])">
	<table border="0" width="100%">
	<xsl:for-each select="m:div[
		(position() mod 5 = 1) and
		(position() &gt; $startsheet) and
		(position() &lt; $endsheet)
	]">
       <tr>
		<xsl:apply-templates 
		select=". | following::m:div[m:div/m:fptr][position() &lt; 5]" 
		mode ="innersheet"/>
	</tr>
	</xsl:for-each>
	</table>
...
<xsl:template match="m:div" mode="innersheet">
...
<xsl:choose>
 <xsl:when test="not(m:div/m:fptr)">
	...
	<xsl:choose>
	  <xsl:when test="count($page/m:mets/m:structMap/m:div/m:div) &gt; 5">
	<xsl:apply-templates select="m:div[position() &lt; 6]" mode="structMap"/>
	<div><a href="/{$page/m:mets/@OBJID}/?sheet=1">more
	 ...</a>
</div>
	  </xsl:when>
	  <xsl:otherwise>
 ...
 <!-- simple image object; should change to link to order= views of images  -->
...
<a href="/{$page/m:mets/@OBJID}/{m:div[starts-with(@TYPE,'reference')]/m:fptr/@FILEID}">Large Image</a>
 ...
 <!-- still toc (div mode structMap) inner divs --> 
 <xsl:otherwise>
	<xsl:variable name="ltype">
		<xsl:choose>
		  <xsl:when test="number($divDepth) &gt; 3">dorder</xsl:when>
	...
	<xsl:text>/</xsl:text>
	<xsl:value-of select="$page/m:mets/@OBJID"/>
	<xsl:text>/?</xsl:text><xsl:value-of select="$ltype"/><xsl:text>=</xsl:text>
        <xsl:value-of select="count( preceding::m:div[m:div/m:fptr] | ancestor::m:div[m:div/m:fptr])+1"/>
	<xsl:if test="$size">
	<xsl:text>&amp;size=</xsl:text>
	<xsl:value-of select="$size"/>
...
<xsl:template match="m:div" mode="structMap-j">
	...
	<xsl:value-of select="$size"/>
        ...
        <xsl:if test="@LABEL != $page/m:mets/@LABEL">
        <a href="{$link}"><img border="0" width="m:div[start-with(@TYPE,'thumbnail')][1]/m:fptr/@cdl2:X" height="m:div[start-with(@TYPE,'thumbnail')][1]/m:fptr/@cdl2:Y" src="{$naillink}"/>
        <br/>
	<!-- xsl:copy-of select="."/ -->
        <xsl:value-of select="@LABEL"/>
        </a>
